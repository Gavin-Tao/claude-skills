...
## Applied / Systems Introduction Pattern (Field-Tested)

A reusable six-paragraph flow for applied ML/RL/control/systems papers whose contribution is a problem reformulation plus a controlled empirical study. Each paragraph carries one role, and the gap and the proposal are explicit, locatable sentences rather than buried clauses.

### Paragraph roles

1. Domain hook (opening): name the application field with 2-3 concrete deployment scenarios, then the operating constraint. If applicable, close by stating that the problem is two-sided / multi-objective.
2. Method-family appeal and the naive-objective failure: why the chosen paradigm (e.g., RL) fits, then "However," the model optimizes exactly the naive objective and exhibits a concrete, costly failure.
3. Established remedy and brittleness = the gap: the standard fix, then point by point why it fails (retune per setting, no guarantee, can backfire). Close with one explicit sentence, e.g., "This is the core gap we address: ...".
4. Insight and proposal: "To bridge this gap, our key observation is that ... Accordingly, this paper formulates/proposes ...". Reframe the problem, name the mechanism, and contrast with what the old fix structurally cannot do.
5. Controlled study / instantiation: the platform, the controlled comparison (what is held fixed, what varies), and the conditions (vehicles/datasets/tasks) with their count.
6. Results preview: open with a consistency statement (e.g., "The evidence is consistent across all [N] settings."), then the headline numbers, a secondary property, the honest exception, and the contrast that isolates the cause.

Then: contributions (bold descriptive titles) and the organization paragraph.

### Connective and honesty conventions

1. Same prose conventions as the Abstract pattern (see `abstract.md` "Prose Conventions"): no em-dash between sentences, "However,"/"Therefore," pivots, "Experiments show that", neutral verbs, explicit gap sentence.
2. Open the proposal with "Therefore/Accordingly, this paper proposes/formulates ...".
3. Contribution bullets use bold descriptive noun-phrase titles (e.g., "Energy-as-constraint formulation for X."), one substantive claim each, and NO specific numbers (numbers belong in the abstract and experiments).
4. Preview the honest exception in the results paragraph instead of hiding it; a stated limitation reads as credible.
5. Mirror the sentence patterns of a supplied reference paper at the structural level only; never copy its prose.

This pattern is the Part A (Version 2/4) + Technical-Challenge (Version 1) + Pipeline (Version 4) chain above, specialized and made concrete for an applied paper with a controlled study.
