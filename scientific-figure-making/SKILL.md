...
## Platform font note (Windows)

The repo demos hard-code `font.family='helvetica'` (authors on macOS). On Windows, Helvetica is
usually NOT installed and matplotlib silently falls back to DejaVu Sans — heavier, awkward
typography plus `findfont` warnings. Set the font via the canonical mechanism with Arial first:
...
Do NOT set `font.family` to a bare font-name list — it still falls back. After rendering, verify
no `findfont` warning was emitted. Also write script-generated figures into a dedicated output
folder (e.g. `figures/`), never the project root, and save redraws of existing figures under new
`*_v2` filenames instead of overwriting originals.
